...
public class Archs1Add extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_archs1_add);

        final String reg_no = getIntent().getStringExtra("reg_no");

        database = FirebaseDatabase.getInstance().getReference();

        database.child("Result").child("Architecture").child(reg_no).child("S1");
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.child(reg_no).child("S1").child("1001").getValue() != null && dataSnapshot.child(reg_no).child("S1").child("1002").getValue() !=null
                    ...
                    if (sub1cp >= 0 && sub2cp >= 0 && sub3cp >= 0 && sub4cp >= 0 && sub5cp >= 0 && sub6cp >= 0) {
                        ...
                        database.child("Result").child("Architecture").child(reg_no).child("S1").child("ttlsscore").setValue(ttl_score);

                        database.child("Result").child("Architecture").child(reg_no).child("S1").child("SGPA").setValue(finalresult);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (Grades.contains(s1) && Grades.contains(s2) && Grades.contains(s3) && Grades.contains(s4) &&Grades.contains(s5) &&Grades.contains(s6)) {
                    Log.d(TAG, "Inside");
                    database.child("Result").child("Architecture").child(reg_no).child("S1").child(c1).setValue(s1);
                    database.child("Result").child("Architecture").child(reg_no).child("S1").child(c2).setValue(s2);
                    database.child("Result").child("Architecture").child(reg_no).child("S1").child(c3).setValue(s3);
                    database.child("Result").child("Architecture").child(reg_no).child("S1").child(c4).setValue(s4);
                    database.child("Result").child("Architecture").child(reg_no).child("S1").child(c5).setValue(s5);
                    database.child("Result").child("Architecture").child(reg_no).child("S1").child(c6).setValue(s6);
