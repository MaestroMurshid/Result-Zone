...
public class MaddStudent extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_m_add_student);
        ...
        database1 = FirebaseDatabase.getInstance().getReference().child("Student").child("Mechanical");
    }
    ...
    public void submit(View view){
        ...
        database.child(students.getReg_no()).setValue(students).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    database.child(Regi_no).child("Department").setValue("Mechanical");
                    Toast.makeText(MaddStudent.this, "Student Registered Successfully", Toast.LENGTH_SHORT).show();
                    startActivity(new Intent(getApplicationContext(), MStudents.class));
                }
                else{
                    Toast.makeText(MaddStudent.this, "Registration Unsuccessful", Toast.LENGTH_SHORT).show();
                }
            }
        });
